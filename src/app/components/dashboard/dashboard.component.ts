...
*/
  ...
  loadComplaints(): void {
    ...
    this.complaintService.getMyComplaints().subscribe({
      ...
      error: () => {
        this.listErrorMessage = 'Erreur lors du chargement des réclamations.';
        this.isLoading = false;
      },
    });
  }
  ...
  calculateDashboardStats(): void {
    ...
    this.inProgressComplaints = this.complaints.filter((c) =>
      c.statut?.toLowerCase().includes('en cours')
    ).length;
  ...
  loadAllStatuses(): void {
    this.complaintService.getAllStatuses().subscribe({
      next: (data) => {
  ...
  onAddComplaint(form: NgForm): void {
    ...
    const complaintData = {
      ...
      sousCategorieId: form.value.sousThemeId,
    };
    ...
    this.complaintService.createComplaint(complaintData).subscribe({
      ...
      error: () =>
        this.openSnackBar('Erreur lors de la création.', 'error-snackbar'),
    });
  }
  ...
  onAddComment(form: NgForm): void {
    ...
    const commentData = {
      ...
      estPrive: this.isUserAdmin ? form.value.estPrive || false : false,
    };

    this.complaintService
      .addComment(this.selectedComplaint.id, commentData)
      .subscribe({
        next: () => {
          ...
          this.complaintService
            .getComments(this.selectedComplaint.id)
            .subscribe({
              next: (c) => {
                this.comments = c;
              },
  ...
  onUpdateStatus(): void {
    ...
    this.complaintService
      .updateStatus(this.selectedComplaint.id, this.selectedStatusId)
      .subscribe({
        next: () => {
          this.openSnackBar('Statut mis à jour !');

          const newStatus = this.allStatuses.find(
            (s) => s.id === this.selectedStatusId
          );
        ...
        error: () =>
          this.openSnackBar('Erreur lors de la mise à jour.', 'error-snackbar'),
      });
  }
  ...
  onDeleteComplaint(complaintId: number, event: MouseEvent): void {
    ...
    if (
      ...
      this.complaintService.deleteComplaint(complaintId).subscribe({
        ...
        error: () =>
          this.openSnackBar('Erreur lors de la suppression.', 'error-snackbar'),
      });
    }
  }
  ...
  openSnackBar(message: string, panelClass: string = 'success-snackbar') {
  ...
  getStatutClass(statut: string): string {
    ...
    if (s.includes('traité') || s.includes('fermé') || s.includes('résolu'))
  ...
  canDeleteComment(comment: any): boolean {
    ...
    const isPending =
      this.selectedComplaint.statut?.nom.toLowerCase() === 'en attente';
